...
m:addOverride("xi.garrison.getAllyInfo", function(zoneData, regionIndex)
    local allyName    = allyNames[zoneData.levelCap][regionIndex]
    local allyLooks   = allyLooks[zoneData.levelCap][regionIndex]
    local allyGroupId = allyGroupIds[zoneData.levelCap]

    for index, allyLook in pairs(allyLooks) do
        allyLooks[index] = addWeaponIfNecessary(allyLook)
    end
