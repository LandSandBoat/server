...
m:addOverride("xi.zones.Nashmau.npcs.Paparoon.onTrade", function(player, npc, trade)
    ...
    -- exchange alexandrite for alex if we're at 30000 --
        if exchangeVoucher == 1 and newStoredAlex < 10000 then
            player:tradeComplete()
            player:setCharVar("Paparoon_AlexStored", (newStoredAlex - 10000))
            player:setCharVar("PendingMythic", 1)
            player:PrintToPlayer("Paparoon: Aight dawg! A deal's a deal! Head back to that creepy ass Tombstone in Caedarva Mire and trade", 0xD)
            player:PrintToPlayer("Paparoon: the corresponding base mythic weapon to get your new bling.", 0xD)
        elseif (newStoredAlex >= 30000 and hasCaptain) then
            player:tradeComplete()
...
m:addOverride("xi.zones.Nashmau.npcs.Paparoon.onTrigger", function(player, npc)
    ...
    if exchangeVoucher == 2 and storedAlex < 10000 then
        player:PrintToPlayer("Paparoon: Yo! What's this?! An exchange voucher? Deposit 10,000 alexandrite and I'll grant your new bling!", 0xD)
		player:PrintToPlayer(string.format("Paparoon: You have %i alexandrite stored!", storedAlex), 0xD)
        return
    elseif exchangeVoucher == 2 and storedAlex >= 10000 then
...
m:addOverride("xi.zones.Caedarva_Mire.npcs.Seaprinces_Tombstone.onTrade", function(player, npc, trade)
    ...
    local weapon = 18990
    for i = 1, 20 do -- All possible mythic weapons
--	    printf("%s", weapon)
        if npcUtil.tradeHasExactly(trade, { weapon }) then
            player:tradeComplete()
            player:setCharVar("MythicWeaponExchange", 1)
            player:PrintToPlayer("???: Hail, adventurer! Hath thou decided to forge a new myth? I will strip this one down for the essential", 0xD)
            player:PrintToPlayer("???: materials required to forge the new one, but you must seek a new base weapon from Nonoroon, and receive further instruction.", 0xD)
		end
