# Generate C++ and Lua settings files from the <root>/settings/settings.template file.
...
def write_lua_settings_files(headers):
    for current_header, header_entries in headers.items():
        with open(f"../settings/default/{current_header}.lua", "w") as fd:
            # Header
            output = f"""-- This file is autogenerated!\n
xi = xi or {{}}
...
def main():
    list = []
    with open('../settings/default/settings.template', 'r') as fd:
        list = fd.readlines()
