...
def needs_to_run(cur):
    ...
    cur.execute(
        "SELECT value FROM char_vars WHERE varname LIKE('HpTeleportMask%') LIMIT 1;"
    )
    ...
    for ids in rows:
        ...
        for charid in range(minID, maxID + 1):
            for x in range(1, 5):
                ...
                right = 0
                cur.execute(
                    "SELECT value FROM char_vars WHERE charid = {} AND varname = 'HpTeleportMask{}a'".format(
                        charid, x
                    )
                )
                rows = cur.fetchall()
                for row in rows:
                    left = row[0]
                cur.execute(
                    "SELECT value FROM char_vars WHERE charid = {} AND varname = 'HpTeleportMask{}b'".format(
                        charid, x
                    )
                )
                rows = cur.fetchall()
            ...
            try:
                cur.execute(
                    "UPDATE char_unlocks SET homepoints = %s WHERE charid = %s",
                    (hpData, charid),
                )
                db.commit()
