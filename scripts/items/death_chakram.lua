...
itemObject.onItemUse = function(target, user)
    if target:hasEquipped(xi.item.DEATH_CHAKRAM) then
        target:addStatusEffect(xi.effect.ENCHANTMENT, { duration = 180, origin = user, sourceType = xi.effectSourceType.EQUIPPED_ITEM, sourceTypeParam = xi.item.DEATH_CHAKRAM })
    end
end
