...
itemObject.onItemUse = function(target, user)
    if target:hasEquipped(xi.item.ELDRITCH_BONE_HAIRPIN) then
        target:addStatusEffect(xi.effect.ENCHANTMENT, { duration = 1800, origin = user, sourceType = xi.effectSourceType.EQUIPPED_ITEM, sourceTypeParam = xi.item.ELDRITCH_BONE_HAIRPIN })
    end
end
