...
entity.onTrigger = function(player, npc)
    if
        player:getCharVar('UnderOathCS') == 5 and
        player:hasKeyItem(xi.keyItem.STRANGE_SHEET_OF_PAPER) and
    ...
    elseif
        ...
        player:hasKeyItem(xi.keyItem.KNIGHTS_CONFESSION)
    then
...
entity.onEventFinish = function(player, csid, option, npc)
    if csid == 113 then
        player:confirmTrade()
        npcUtil.giveKeyItem(player, xi.keyItem.KNIGHTS_CONFESSION)
        player:setCharVar('UnderOathCS', 6)
        player:delKeyItem(xi.keyItem.STRANGE_SHEET_OF_PAPER)
    end
end
