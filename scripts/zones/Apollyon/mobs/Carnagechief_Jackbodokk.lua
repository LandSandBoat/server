...
entity.onMobFight = function(mob, target)
    ...
    if battlefield then
        ...
        if
            GetMobByID(ID.mob.APOLLYON_CS_MOB[1] + 1):isDead() and
            GetMobByID(ID.mob.APOLLYON_CS_MOB[1] + 2):isDead() and
            GetMobByID(ID.mob.APOLLYON_CS_MOB[1] + 3):isDead() and
            wave == 1
        then
        ...
        elseif
            ...
            GetMobByID(ID.mob.APOLLYON_CS_MOB[1] + 5):isDead() and
            GetMobByID(ID.mob.APOLLYON_CS_MOB[1] + 6):isDead() and
            wave == 2
        then
            mob:setLocalVar("wave", 1)
        ...
        if remainingTime <= startTime * 0.66 then
            ...
            elseif
                GetMobByID(ID.mob.APOLLYON_CS_MOB[2]):isAlive() and
                not GetMobByID(ID.mob.APOLLYON_CS_MOB[2]):isEngaged()
            then
                battlefield:setLocalVar("startTime", battlefield:getRemainingTime())
...
entity.onMobDeath = function(mob, player, isKiller, noKiller)
    if isKiller or noKiller then
        ...
        elseif
            GetMobByID(ID.mob.APOLLYON_CS_MOB[3]):isAlive() and
            GetMobByID(ID.mob.APOLLYON_CS_MOB[2]):isAlive()
        then
