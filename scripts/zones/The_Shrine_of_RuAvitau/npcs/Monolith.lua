...
entity.onTrigger = function(player, npc)
    ...
    if offset >= 0 and offset <= 38 then
        ...
        for i = 0, 21 do
            local anim = doors[i] == colorTouched and xi.animation.OPEN_DOOR or xi.animation.CLOSE_DOOR
            GetNPCByID(ID.npc.DOOR_OFFSET + i):setAnimation(anim)
        end

        for i = 0, 19 do
            local anim = monoliths[i] == colorTouched and xi.animation.OPEN_DOOR or xi.animation.CLOSE_DOOR
