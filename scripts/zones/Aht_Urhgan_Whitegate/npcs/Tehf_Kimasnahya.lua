...
entity.onTrigger = function(player, npc)
    local gotitall = player:getQuestStatus(xi.quest.log_id.AHT_URHGAN, xi.quest.id.ahtUrhgan.GOT_IT_ALL)
...
entity.onEventFinish = function(player, csid, option)
    if csid == 520 then
        player:addQuest(xi.quest.log_id.AHT_URHGAN, xi.quest.id.ahtUrhgan.GOT_IT_ALL)
    ...
    elseif csid == 528 then
        npcUtil.completeQuest(player, AHT_URHGAN, xi.quest.id.ahtUrhgan.GOT_IT_ALL, {item=18257, var={"Wait1DayForgotitallCS_date", "gotitallCS"}})
