...
entity.onTrigger = function(player, npc)
    if player:getCharVar('BathedInScent') == 1 then
        if player:getQuestStatus(xi.quest.log_id.OUTLANDS, xi.quest.id.outlands.PERSONAL_HYGIENE) == xi.questStatus.QUEST_AVAILABLE then
            player:startEvent(191)
        elseif player:getQuestStatus(xi.quest.log_id.OUTLANDS, xi.quest.id.outlands.PERSONAL_HYGIENE) == xi.questStatus.QUEST_ACCEPTED then
            player:startEvent(192)
        else
            player:startEvent(195)
        end
    elseif
        player:getQuestStatus(xi.quest.log_id.OUTLANDS, xi.quest.id.outlands.PERSONAL_HYGIENE) == xi.questStatus.QUEST_ACCEPTED and
