...
function onTrigger(player, npc)
    if player:getCharVar("BathedInScent") == 1 then
        ...
        elseif (player:getQuestStatus(tpz.quest.log_id.OUTLANDS, tpz.quest.id.outlands.PERSONAL_HYGIENE) == QUEST_ACCEPTED) then
            player:startEvent(192)
        else
            player:startEvent(195)
        end
    elseif (player:getQuestStatus(tpz.quest.log_id.OUTLANDS, tpz.quest.id.outlands.PERSONAL_HYGIENE) == QUEST_ACCEPTED and player:getCharVar("BathedInScent") == 0) then
