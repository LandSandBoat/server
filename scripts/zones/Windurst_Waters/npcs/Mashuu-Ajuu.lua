...
entity.onTrade = function(player, npc, trade)
    ...
    if reapstatus >= 1 and trade:getItemCount() == 1 and trade:getGil() == 0 then
        if trade:hasItemQty(xi.items.SOBBING_FUNGUS, 1) then
            player:startEvent(475, 500, xi.items.STATIONERY_SET)                     -- REAP WHAT YOU SOW + GIL: Quest Turn In: Sobbing Fungus turned in
        elseif trade:hasItemQty(xi.items.DEATHBALL, 1) then
...
entity.onTrigger = function(player, npc)
    ...
    if reapstatus == QUEST_AVAILABLE then
        ...
        if rand == 1 then
            player:startEvent(463, 0, xi.items.SOBBING_FUNGUS, xi.items.BAG_OF_HERB_SEEDS)                 -- REAP WHAT YOU SOW + HERB SEEDS: QUEST START
        end
    elseif reapstatus == QUEST_ACCEPTED then
        local rand = math.random(1, 2)
        if rand == 1 then
            player:startEvent(464, 0, xi.items.SOBBING_FUNGUS, xi.items.BAG_OF_HERB_SEEDS)                  -- REAP WHAT YOU SOW + HERB SEEDS: OBJECTIVE REMINDER
        else
    ...
    then
        local rand = math.random(1, 2)
        if rand == 1 then
            player:startEvent(479, 0, xi.items.SOBBING_FUNGUS, xi.items.BAG_OF_HERB_SEEDS)                -- REAP WHAT YOU SOW + HERB SEEDS: REPEATABLE QUEST START
        end
    elseif
        reapstatus == QUEST_COMPLETED and
