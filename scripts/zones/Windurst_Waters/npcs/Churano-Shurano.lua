...
entity.onTrigger = function(player, npc)
    if not player:hasKeyItem(xi.keyItem.MAGICKED_ASTROLABE) then
        local cost = 10000
...
entity.onEventUpdate = function(player, csid, option, npc)
    if csid == 1080 or csid == 1081 then
        if option == 1 and player:getGil() >= 10000 then
            player:updateEvent(xi.keyItem.MAGICKED_ASTROLABE)
        else
...
entity.onEventFinish = function(player, csid, option, npc)
    if csid == 1080 and option ~= xi.keyItem.MAGICKED_ASTROLABE then
        player:setLocalVar('Astrolabe', 1)
    elseif
        (csid == 1080 or csid == 1081) and
        option == xi.keyItem.MAGICKED_ASTROLABE and
        player:getGil() >= 10000
    then
        npcUtil.giveKeyItem(player, xi.keyItem.MAGICKED_ASTROLABE)
        player:delGil(10000)
    end
end
