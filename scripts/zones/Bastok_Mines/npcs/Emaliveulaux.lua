...
entity.onTrigger = function(player, npc)
    if player:getCurrentMission(xi.mission.log_id.COP) >= xi.mission.id.cop.THE_SAVAGE then
        if GetRegionOwner(xi.region.TAVNAZIANARCH) ~= xi.nation.BASTOK then
            player:showText(npc, zones[xi.zone.BASTOK_MINES].text.EMALIVEULAUX_CLOSED_DIALOG)
        else
            local stock =
            {
                { 1523,  290 }, -- Apple Mint
            ...
            player:showText(npc, zones[xi.zone.BASTOK_MINES].text.EMALIVEULAUX_OPEN_DIALOG)
            xi.shop.general(player, stock, xi.fameArea.BASTOK)
        end
    else
        player:showText(npc, zones[xi.zone.BASTOK_MINES].text.EMALIVEULAUX_COP_NOT_COMPLETED)
    end
end
