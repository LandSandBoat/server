...
-- Consider moving this file into Modules?
        ...
        for _, eggID in ipairs(eggTypes) do
            if npcUtil.tradeHasExactly(trade, { itemID, eggID }) then
                ...
                if slots and descriptions then
                    ...
                    if itemID == xi.item.RIDILL then
                        itemName = 'your Ridill'
                    elseif itemID == xi.item.KRAKEN_CLUB then
                        itemName = 'your Kraken Club'
                    elseif itemID == xi.item.JOYEUSE then
                        itemName = 'your Joyeuse'
                    elseif itemID == xi.item.MERCURIAL_KRIS then
                        itemName = 'your Mercurial Kris'
                    elseif itemID == xi.item.CHATOYANT_STAFF then
                        itemName = 'your Chatoyant Staff'
                    elseif itemID == xi.item.SCORPION_HARNESS_P1 then
