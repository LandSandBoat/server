...
entity.onTrigger = function(player, npc)
    ...
    if tomath == xi.questStatus.QUEST_ACCEPTED then
        if player:hasKeyItem(xi.keyItem.EXTRAVAGANT_HARPOON) then
...
entity.onEventFinish = function(player, csid, option, npc)
    if csid == 2540 then
        ...
        npcUtil.giveKeyItem(player, xi.keyItem.BROKEN_HARPOON)
    elseif csid == 2542 then
        ...
        player:delKeyItem(xi.keyItem.EXTRAVAGANT_HARPOON)
