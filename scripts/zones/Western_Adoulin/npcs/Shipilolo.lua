...
entity.onTrigger = function(player, npc)
    ...
    if player:getCurrentMission(xi.mission.log_id.SOA) >= xi.mission.id.soa.LIFE_ON_THE_FRONTIER then
        ...
        elseif
            ...
            not player:hasKeyItem(xi.keyItem.WAYPOINT_RECALIBRATION_KIT)
        then
...
entity.onEventFinish = function(player, csid, option, npc)
    if csid == 2543 then
        -- Progresses Quest: 'The Old Man and the Harpoon'
        player:delKeyItem(xi.keyItem.BROKEN_HARPOON)
        npcUtil.giveKeyItem(player, xi.keyItem.EXTRAVAGANT_HARPOON)
    elseif csid == 2850 then
        -- Progresses Quest: 'Fertile Ground' TODO: Should this also give the player a message?
        player:addKeyItem(xi.keyItem.BOTTLE_OF_FERTILIZER_X)
    elseif csid == 79 then
        player:addKeyItem(xi.keyItem.WAYPOINT_RECALIBRATION_KIT)
