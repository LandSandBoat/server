...
entity.onTrigger = function(player, npc)
    ...
    local guildMember       = xi.crafting.hasJoinedGuild(player, xi.guild.WOODWORKING) and 150995375 or 0
...
entity.onEventFinish = function(player, csid, option, npc)
    if csid == 621 and option == 2 then
        if xi.crafting.hasJoinedGuild(player, xi.guild.WOODWORKING) then
    ...
    elseif csid == 621 and option == 1 then
        if player:getFreeSlotsCount() == 0 then
            ...
            xi.crafting.signupGuild(player, xi.guild.WOODWORKING)
        end
    else
