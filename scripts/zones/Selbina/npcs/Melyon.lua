...
entity.onTrade = function(player, npc, trade)
    if player:getQuestStatus(xi.questLog.OTHER_AREAS, xi.quest.id.otherAreas.ONLY_THE_BEST) ~= xi.questStatus.QUEST_AVAILABLE then
        if npcUtil.tradeHas(trade, { { xi.item.LA_THEINE_CABBAGE, 5 } }) then -- La Theine Cabbage x5
            player:startEvent(62, 0, xi.item.LA_THEINE_CABBAGE)
        elseif npcUtil.tradeHas(trade, { { xi.item.EAR_OF_MILLIONCORN, 3 } }) then -- Millioncorn x3
            player:startEvent(63, 0, xi.item.EAR_OF_MILLIONCORN)
...
entity.onEventFinish = function(player, csid, option, npc)
    ...
    elseif csid == 62 and option == 11 then
        ...
        player:addFame(xi.fameArea.BASTOK, 4)
        player:addFame(xi.fameArea.SANDORIA, 4)
    ...
    elseif csid == 63 and option == 12 then
        ...
        player:addFame(xi.fameArea.BASTOK, 6)
        player:addFame(xi.fameArea.SANDORIA, 6)
        player:completeQuest(xi.questLog.OTHER_AREAS, xi.quest.id.otherAreas.ONLY_THE_BEST)
        player:confirmTrade()
    elseif csid == 64 and option == 13 then
        npcUtil.giveCurrency(player, 'gil', 600)
        player:addFame(xi.fameArea.BASTOK, 8)
        player:addFame(xi.fameArea.SANDORIA, 8)
        player:completeQuest(xi.questLog.OTHER_AREAS, xi.quest.id.otherAreas.ONLY_THE_BEST)
        player:confirmTrade()
    end
end
