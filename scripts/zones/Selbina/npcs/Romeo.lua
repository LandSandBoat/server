...
entity.onEventFinish = function(player, csid, option, npc)
    ...
    elseif
        csid == 21 and
        npcUtil.completeQuest(player, xi.questLog.OTHER_AREAS, xi.quest.id.otherAreas.DONATE_TO_RECYCLING, { item = 89, fame_area = xi.quest.fame_area.SELBINA_RABAO, title = xi.title.ECOLOGIST })
    then
        player:confirmTrade()
    end
end
