...
entity.onTrade = function(player, npc, trade)
    ...
    local PrettyLittleThings = player:getQuestStatus(xi.quest.log_id.JEUNO, xi.quest.id.jeuno.PRETTY_LITTLE_THINGS)
...
entity.onEventFinish = function(player, csid, option)
    ...
    if (csid == 10023 and option == 4002) then
        ...
        player:completeQuest(xi.quest.log_id.JEUNO, xi.quest.id.jeuno.PRETTY_LITTLE_THINGS)
    elseif (csid == 10023 and option == 1) then
        player:tradeComplete()
        player:addQuest(xi.quest.log_id.JEUNO, xi.quest.id.jeuno.PRETTY_LITTLE_THINGS)
    end
end
