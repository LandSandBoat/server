...
entity.onTrigger = function(player, npc)
    ...
    local hasWhisperOfTides = player:hasKeyItem(xi.keyItem.WHISPER_OF_TIDES)
    ...
    then
        player:startEvent(109, 0, xi.keyItem.TUNING_FORK_OF_WATER) -- Start and restart quest 'Trial by Water'
    elseif
        trialByWater == xi.questStatus.QUEST_ACCEPTED and
        not player:hasKeyItem(xi.keyItem.TUNING_FORK_OF_WATER) and
        not hasWhisperOfTides
    then
        player:startEvent(190, 0, xi.keyItem.TUNING_FORK_OF_WATER) -- Defeat against Avatar : Need new Fork
    elseif trialByWater == xi.questStatus.QUEST_ACCEPTED and not hasWhisperOfTides then
        player:startEvent(110, 0, xi.keyItem.TUNING_FORK_OF_WATER, 2)
    elseif trialByWater == xi.questStatus.QUEST_ACCEPTED and hasWhisperOfTides then
        ...
        player:startEvent(112, 0, xi.keyItem.TUNING_FORK_OF_WATER, 2, 0, numitem)
    else
...
entity.onEventFinish = function(player, csid, option, npc)
    if csid == 109 and option == 1 then
        ...
        npcUtil.giveKeyItem(player, xi.keyItem.TUNING_FORK_OF_WATER)
    ...
    elseif csid == 112 then
        ...
        if player:getFreeSlotsCount() == 0 and (option ~= 5 or option ~= 6) then
            ...
            player:delKeyItem(xi.keyItem.WHISPER_OF_TIDES) --Whisper of Tides, as a trade for the above rewards
