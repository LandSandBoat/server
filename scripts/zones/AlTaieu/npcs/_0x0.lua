...
entity.onTrigger = function(player, npc)
    ...
    if (player:getCurrentMission(COP) == xi.mission.id.cop.GARDEN_OF_ANTIQUITY and player:getCharVar("PromathiaStatus") == 1) then
        player:startEvent(164)
    elseif (player:getCurrentMission(COP) > xi.mission.id.cop.GARDEN_OF_ANTIQUITY or (player:getCurrentMission(COP) == xi.mission.id.cop.GARDEN_OF_ANTIQUITY and player:getCharVar("PromathiaStatus") == 3)) then
