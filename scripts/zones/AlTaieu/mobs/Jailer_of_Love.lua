...
local spellLists =
{
    [xi.element.FIRE]    = 508, -- Fire List
    [xi.element.ICE]     = 509, -- Ice List
    [xi.element.WIND]    = 510, -- Wind List
    [xi.element.EARTH]   = 511, -- Earth List
    [xi.element.THUNDER] = 512, -- Thunder List
    [xi.element.WATER]   = 513, -- Water List
    [xi.element.LIGHT]   = 514, -- Light List
    [xi.element.DARK]    = 515, -- Dark List
}
...
entity.onMobInitialize = function(mob)
    mob:setBehaviour(xi.behavior.STANDBACK)
...
entity.onMobEngage = function(mob, target)
    ...
    mob:setLocalVar('currentAbsorb', math.random(#xi.combat.element.absorbMod)) -- pick a random element to absorb after engaging
    mob:setSpellList(spellLists[mob:getLocalVar('currentAbsorb')])
    mob:setMod(xi.combat.element.absorbMod[mob:getLocalVar('currentAbsorb')], 100)
end
...
entity.onMobFight = function(mob, target)
    ...
    if os.time() > mob:getLocalVar('elementAbsorb') then
        ...
        if previousAbsorb > 0 then
            mob:setMod(xi.combat.element.absorbMod[previousAbsorb], 0)
        end

        -- add new absorb mod
        local currentAbsorb = math.random(#xi.combat.element.absorbMod)
        ...
        mob:setMod(xi.combat.element.absorbMod[currentAbsorb], 100)
