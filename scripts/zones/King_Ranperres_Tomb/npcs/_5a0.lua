...
entity.onTrigger = function(player, npc)
    ...
    if
        currentMission == xi.mission.id.sandoria.RANPERRES_FINAL_REST and
        missionStatus == 1 and
    ...
    if currentMission == xi.mission.id.sandoria.RANPERRES_FINAL_REST and missionStatus == 2 then -- NMs killed
        player:setMissionStatus(player:getNation(), 3)
    elseif currentMission == xi.mission.id.sandoria.RANPERRES_FINAL_REST and missionStatus == 3 and player:getXPos() > -39.019 then -- standing outside
        player:startEvent(6) -- enter cutscene
    elseif currentMission == xi.mission.id.sandoria.RANPERRES_FINAL_REST and missionStatus == 3 then -- inside
        player:startEvent(7) -- exit cutscene
    elseif currentMission == xi.mission.id.sandoria.RANPERRES_FINAL_REST and missionStatus == 6 then
        player:startEvent(5)
