...
entity.onMobSpawn = function(mob)
    ...
    -- do not need to set res rank for earth because WTB primes have own
...
entity.onAdditionalEffect = function(mob, target, damage)
    return xi.mob.onAddEffect(mob, target, damage, xi.mob.ae.ENSTONE, { chance = 100, power = math.random(75, 125) })
end

entity.onMobWeaponSkill = function(target, mob, skill)
    -- if not all four elementals are alive then respawn one after using level 75 BP
    if skill:getID() == 856 then
    ...
    if mob:getLocalVar('healTimer') < os.time() then
        ...
        for i = 1, 4 do
            ...
            if elemental and elemental:isAlive() then
                ...
                if spellTarget then
                    elemental:castSpell(xi.magic.spell.STONE_IV, spellTarget)
                    mob:setLocalVar('healTimer', os.time() + math.random(30, 60))
                    break
                end
            end
