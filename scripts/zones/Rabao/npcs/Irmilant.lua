...
entity.onEventFinish = function(player, csid, option, npc)
    ...
    elseif
        csid == 78 and
        npcUtil.completeQuest(player, xi.questLog.OUTLANDS, xi.quest.id.outlands.THE_IMMORTAL_LU_SHANG, { item = 17386, title = xi.title.THE_IMMORTAL_FISHER_LU_SHANG })
    then
    ...
    elseif csid == 134 then
        if npcUtil.completeQuest(player, xi.questLog.OUTLANDS, xi.quest.id.outlands.INDOMITABLE_SPIRIT, { item = 17011, title = xi.title.INDOMITABLE_FISHER, var = 'IndomitableSpiritTimer' }) then
            player:addFame(xi.fameArea.SANDORIA, 10)
            player:addFame(xi.fameArea.BASTOK, 10)
        end
    end
