...
--- If the player is dead, already charmed or is no longer preset in the battlefield, Osschaart will skip to the next player in the order.
...
local function charm(mob)
    ...
    -- Copy two-hour ability of charmed player and have it ready for use after a short delay
    ...
    if twoHour then
        ...
        mob:setLocalVar('twoHourCopiedTime', GetSystemTime())
    end
...
-----------------------------------
--- MobInitialize: Sets Astral Flow pet offset, double attack, and wind resistance rank.
-----------------------------------
...
entity.onMobFight = function(mob, target)
    ...
    -- Then check if we should use the copied two-hour ability (with minimum 3 second delay after copying)
    local twoHourCopiedTime = mob:getLocalVar('twoHourCopiedTime')
    if
        mobHPP <= twoHourHPP and
        mob:getLocalVar('copiedTwoHour') > 0 and
        twoHourUsed == 0 and
        (twoHourCopiedTime == 0 or currentTime >= twoHourCopiedTime + 3)
    then
