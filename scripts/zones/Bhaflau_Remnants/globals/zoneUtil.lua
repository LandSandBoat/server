    ...
    if instance then
        ...
        if floor == 1 then
            if utils.contains(mobID, ID.mob.WAMOURACAMPA) then
                return equipCells[math.randomInt(1, #equipCells)],
                equipCells[math.randomInt(1, #equipCells)],
                equipCells[math.randomInt(1, #equipCells)]
            elseif utils.contains(mobID, ID.mob.CARMINE_ERUCA) then
                return statCells[math.randomInt(1, #statCells)],
                statCells[math.randomInt(1, #statCells)],
                statCells[math.randomInt(1, #statCells)]
            else
                return xi.item.INCUS_CELL, xi.item.DUPLICATUS_CELL,
                specialCells[math.randomInt(1, #specialCells)]
            end
        end
