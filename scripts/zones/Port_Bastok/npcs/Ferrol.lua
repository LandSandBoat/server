...
entity.onTrade = function(player, npc, trade)
    if
        trade:hasItemQty(xi.items.MINI_TUNING_FORK_OF_EARTH, 1) and
    ...
    then
        player:startEvent(298, 0, xi.items.MINI_TUNING_FORK_OF_EARTH, 1, 20)
    end
end
...
entity.onTrigger = function(player, npc)
    ...
    then
        ...
        player:startEvent(297, 0, xi.items.MINI_TUNING_FORK_OF_EARTH, 1, 20)     --mini tuning fork, zone, level
    elseif trialSizeEarth == QUEST_ACCEPTED then
        ...
        if earthFork then
            ...
            player:startEvent(301, 0, xi.items.MINI_TUNING_FORK_OF_EARTH, 1, 20) -- Need another mini tuning fork
        end
...
entity.onEventFinish = function(player, csid, option)
    if csid == 297 and option == 1 then
        if player:getFreeSlotsCount() == 0 then
            player:messageSpecial(ID.text.ITEM_CANNOT_BE_OBTAINED, xi.items.MINI_TUNING_FORK_OF_EARTH) --Mini tuning fork
        else
            ...
            player:messageSpecial(ID.text.ITEM_OBTAINED, xi.items.MINI_TUNING_FORK_OF_EARTH)
        end
