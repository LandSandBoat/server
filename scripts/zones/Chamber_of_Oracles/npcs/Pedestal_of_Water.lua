...
entity.onTrigger = function(player, npc)
    ...
    if (player:getCurrentMission(ZILART) == xi.mission.id.zilart.THE_CHAMBER_OF_ORACLES) then
        if (player:hasKeyItem(xi.ki.WATER_FRAGMENT)) then
            player:delKeyItem(xi.ki.WATER_FRAGMENT)
            player:setMissionStatus(xi.mission.log_id.ZILART, ZilartStatus + 64)
            player:messageSpecial(ID.text.YOU_PLACE_THE, xi.ki.WATER_FRAGMENT)
        ...
        elseif (ZilartStatus == 255) then -- Execute cutscene if the player is interrupted.
            ...
            player:messageSpecial(ID.text.IS_SET_IN_THE_PEDESTAL, xi.ki.WATER_FRAGMENT)
        end
    elseif (player:hasCompletedMission(xi.mission.log_id.ZILART, xi.mission.id.zilart.THE_CHAMBER_OF_ORACLES)) then
        player:messageSpecial(ID.text.HAS_LOST_ITS_POWER, xi.ki.WATER_FRAGMENT)
    else
