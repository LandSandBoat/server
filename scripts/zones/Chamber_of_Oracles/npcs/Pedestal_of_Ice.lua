...
entity.onTrigger = function(player, npc)
    ...
    if (player:getCurrentMission(ZILART) == xi.mission.id.zilart.THE_CHAMBER_OF_ORACLES) then
        if (player:hasKeyItem(xi.ki.ICE_FRAGMENT)) then
            player:delKeyItem(xi.ki.ICE_FRAGMENT)
            player:setMissionStatus(xi.mission.log_id.ZILART, ZilartStatus + 8)
            player:messageSpecial(ID.text.YOU_PLACE_THE, xi.ki.ICE_FRAGMENT)
        ...
        elseif (ZilartStatus == 255) then -- Execute cutscene if the player is interrupted.
            ...
            player:messageSpecial(ID.text.IS_SET_IN_THE_PEDESTAL, xi.ki.ICE_FRAGMENT)
        end
    elseif (player:hasCompletedMission(xi.mission.log_id.ZILART, xi.mission.id.zilart.THE_CHAMBER_OF_ORACLES)) then
        player:messageSpecial(ID.text.HAS_LOST_ITS_POWER, xi.ki.ICE_FRAGMENT)
    else
