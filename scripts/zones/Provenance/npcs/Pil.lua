    ...
    for itemID, entry in pairs(augments) do
        ...
        if tradeMatches then
            ...
            if slots then
                ...
                if itemID == xi.item.TOREADORS_RING then
                    itemName = 'your Toreador\'s Ring'
                elseif itemID == xi.item.ROSE_STRAP then
                    itemName = 'your Rose Strap'
                elseif itemID == xi.item.VELOCIOUS_BELT then
                    itemName = 'your Velocious Belt'
                elseif itemID == xi.item.LOQUACIOUS_EARRING then
                    itemName = 'your Loquacious Earring'
                elseif itemID == xi.item.BRUTAL_EARRING then
                    itemName = 'your Brutal Earring'
                elseif itemID == xi.item.PRISM_CAPE then
                    itemName = 'your Prism Cape'
                elseif itemID == xi.item.WALAHRA_TURBAN then
                    itemName = 'your Wal-Mart Turban'
                elseif itemID == xi.item.HAUBERGEON_P1 then
                    itemName = 'your Hauby'
                elseif itemID == xi.item.BIBIKI_SEASHELL then
