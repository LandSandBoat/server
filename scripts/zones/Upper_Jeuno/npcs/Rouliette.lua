...
entity.onTrade = function(player, npc, trade)
    if
        player:getQuestStatus(xi.quest.log_id.JEUNO, xi.quest.id.jeuno.CANDLE_MAKING) == QUEST_ACCEPTED and
        trade:hasItemQty(531, 1) == true and
...
entity.onEventFinish = function(player, csid, option)
    if
        csid == 36 and
        player:getQuestStatus(xi.quest.log_id.JEUNO, xi.quest.id.jeuno.CANDLE_MAKING) == QUEST_AVAILABLE
    then
