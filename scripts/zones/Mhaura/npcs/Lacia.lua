...
entity.onTrade = function(player, npc, trade)
    if
        trade:hasItemQty(xi.items.MINI_TUNING_FORK_OF_LIGHTNING, 1) and
    ...
    then
        player:startEvent(10026, 0, xi.items.MINI_TUNING_FORK_OF_LIGHTNING, 5, 20)
    end
end
...
entity.onTrigger = function(player, npc)
    ...
    then
        ...
        player:startEvent(10025, 0, xi.items.MINI_TUNING_FORK_OF_LIGHTNING, 5, 20)     --mini tuning fork of lightning, zone, level
    elseif trialSizeLightning == QUEST_ACCEPTED then
        ...
        if hasLightningFork then
            ...
            player:startEvent(10029, 0, xi.items.MINI_TUNING_FORK_OF_LIGHTNING, 5, 20) --Need another mini tuning fork
        end
...
entity.onEventFinish = function(player, csid, option, npc)
    if csid == 10025 and option == 1 then
        if player:getFreeSlotsCount() == 0 then
            player:messageSpecial(ID.text.ITEM_CANNOT_BE_OBTAINED, xi.items.MINI_TUNING_FORK_OF_LIGHTNING)
        else
