...
entity.onTrigger = function(player, npc)
    if npc:getAnimation() ~= xi.animation.OPEN_DOOR then
        npc:updateToEntireZone(xi.status.NORMAL, xi.animation.OPEN_DOOR)
        local column = GetNPCByID(npc:getID() - 4)
        if column then
            column:updateToEntireZone(xi.status.NORMAL, xi.animation.OPEN_DOOR)
        end
    else
    ...
    if
        GetNPCByID(ID.npc.ALTEPA_GATE + 5):getAnimation() == xi.animation.OPEN_DOOR and
        GetNPCByID(ID.npc.ALTEPA_GATE + 6):getAnimation() == xi.animation.OPEN_DOOR and
        GetNPCByID(ID.npc.ALTEPA_GATE + 7):getAnimation() == xi.animation.OPEN_DOOR and
        GetNPCByID(ID.npc.ALTEPA_GATE + 8):getAnimation() == xi.animation.OPEN_DOOR
    then
        ...
        for i = ID.npc.ALTEPA_GATE, ID.npc.ALTEPA_GATE + 8 do
            ...
            if door then
                door:updateToEntireZone(xi.status.NORMAL, xi.animation.OPEN_DOOR)
                door:timer(openTime, function(doorArg)
                    doorArg:updateToEntireZone(xi.status.NORMAL, xi.animation.CLOSE_DOOR)
                end)
            end
        end
    end
