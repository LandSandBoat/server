...
entity.onTrade = function(player, npc, trade)
	...
	-- exchange alexandrite for alex if we're at 30000 --
		if (newStoredAlex >= 30000 and player:hasKeyItem(xi.ki.CAPTAIN_WILDCAT_BADGE)) then
    	...
    	elseif (newStoredAlex >= 30000 and not player:hasKeyItem(xi.ki.CAPTAIN_WILDCAT_BADGE)) then
		    player:tradeComplete()
...
entity.onTrigger = function(player, npc)
	...
	elseif (storedAlex >= 30000 and not player:hasKeyItem(xi.ki.CAPTAIN_WILDCAT_BADGE)) then 
        player:PrintToPlayer("Paparoon: You still not a captain?! Come back when you are!", 0xD)
	elseif (storedAlex >= 30000 and player:hasKeyItem(xi.ki.CAPTAIN_WILDCAT_BADGE)) then
