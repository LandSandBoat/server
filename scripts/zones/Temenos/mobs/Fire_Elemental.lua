        ...
        if battlefield:getLocalVar("crateOpenedF1") ~= 1 then
            switch (mobID): caseof
            {
                [ID.mob.TEMENOS_E_MOB[1]] = function()
                    ...
                    tpz.limbus.spawnRandomCrate(ID.npc.TEMENOS_E_CRATE[1], player, "crateMaskF1", battlefield:getLocalVar("crateMaskF1"), true)
                end,
                [ID.mob.TEMENOS_E_MOB[1]+1] = function()
                    GetNPCByID(ID.npc.TEMENOS_E_CRATE[1]+1):setPos(mobX, mobY, mobZ)
                    tpz.limbus.spawnRandomCrate(ID.npc.TEMENOS_E_CRATE[1]+1, player, "crateMaskF1", battlefield:getLocalVar("crateMaskF1"), true)
                end,
                [ID.mob.TEMENOS_E_MOB[1]+2] = function()
                    GetNPCByID(ID.npc.TEMENOS_E_CRATE[1]+2):setPos(mobX, mobY, mobZ)
                    tpz.limbus.spawnRandomCrate(ID.npc.TEMENOS_E_CRATE[1]+2, player, "crateMaskF1", battlefield:getLocalVar("crateMaskF1"), true)
                end,
                [ID.mob.TEMENOS_E_MOB[1]+3] = function()
                    GetNPCByID(ID.npc.TEMENOS_E_CRATE[1]+3):setPos(mobX, mobY, mobZ)
                    tpz.limbus.spawnRandomCrate(ID.npc.TEMENOS_E_CRATE[1]+3, player, "crateMaskF1", battlefield:getLocalVar("crateMaskF1"), true)
                end,
