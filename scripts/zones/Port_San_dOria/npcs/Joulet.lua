...
function onTrade(player, npc, trade)
    ...
    if (MoatCarp + ForestCarp > 0 and MoatCarp + ForestCarp == count) then
        if (player:getQuestStatus(tpz.quest.log_id.SANDORIA, tpz.quest.id.sandoria.THE_COMPETITION) == QUEST_ACCEPTED and totalFish >= 10000) then -- ultimate reward
        ...
        elseif (player:getQuestStatus(tpz.quest.log_id.SANDORIA, tpz.quest.id.sandoria.THE_COMPETITION) >= QUEST_ACCEPTED) then -- regular turn-ins. Still allowed after completion of the quest.
            player:tradeComplete()
            player:addFame(SANDORIA, 30)
            player:addGil((GIL_RATE*10*MoatCarp) + (GIL_RATE*15*ForestCarp))
