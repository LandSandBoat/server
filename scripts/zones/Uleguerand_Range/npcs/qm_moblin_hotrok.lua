...
entity.onTrade = function(player, npc, trade)
    ...
    then
        ...
        if player:getMissionStatus(xi.mission.log_id.COP, xi.mission.status.COP.LOUVERANCE) == 14 then
            ...
            then
                player:startEvent(10, 0, 1729, xi.keyItem.MAP_OF_THE_ULEGUERAND_RANGE, 0, 0, 1, 0)
                --                                                                      ^ 1 = Remembers you and girl from Tavnazia and asks to TRADE dagger for map
            else
                player:startEvent(10, 0, 1729, xi.keyItem.MAP_OF_THE_ULEGUERAND_RANGE, 1, 0, 1, 0)
            ...
            if
                ...
                player:hasTitle(xi.title.TRUE_COMPANION_OF_LOUVERANCE)
            then
                player:startEvent(10, 0, 1729, xi.keyItem.MAP_OF_THE_ULEGUERAND_RANGE, 0, 1, 0, 0)
                --                                                                   ^ 1 = Doesn't recognize you and asks to TRADE dagger for map
                -- or
                --                                                                          ^ 1 = Doesn't recognize you and asks to TRADE dagger for map
            else
                player:startEvent(10, 0, 1729, xi.keyItem.MAP_OF_THE_ULEGUERAND_RANGE, 1, 0, 0, 0)
...
entity.onEventFinish = function(player, csid, option, npc)
    if
        ...
        npcUtil.completeQuest(player, xi.questLog.SANDORIA, xi.quest.id.sandoria.OVER_THE_HILLS_AND_FAR_AWAY, {
            ...
            keyItem = xi.keyItem.MAP_OF_THE_ULEGUERAND_RANGE,
            gil = 2000,
            exp = 2000
        })
    then
