...
xi.dynamis.refillMobGroupOnDeath = function(mob, player, isKiller, mobSpawnTime)
    local mobId = mob:getID()
    local zoneId = mob:getZoneID()
    local ID = zones[zoneId]
    local RFMG = ID.mob.REFILL_MOB_GROUP
    ...
    if RFMG then
        ...
        if found then
            ...
            printf("[xi.dynamis.refillMobGroupOnDeath] called in zone %i on mob %i that does not appear in a refill mob group.", zoneId, mobId)
        end
    else
        printf("[xi.dynamis.refillMobGroupOnDeath] called on mob %i in zone %i that does not have a REGILL_MOB_GROUP table in its IDs.", mobId, zoneId)
    end
end
