...
ability_object.onUseAbility = function(player, target, ability)
    ...
    if target:getActiveRunes() > 0 and target:hasStatusEffect(xi.effect.UNDA) then
        local effect = player:getStatusEffect(xi.effect.UNDA)
    ...
    target:addStatusEffect(xi.effect.UNDA,finalDmg,3,180)
    printf("final damage for en effect UNDA = %s", finalDmg)

    return xi.effect.UNDA
end
