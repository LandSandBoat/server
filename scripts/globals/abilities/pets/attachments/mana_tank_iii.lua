...
attachment_object.onEquip = function(pet)
    ...
    if frame == xi.frames.HARLEQUIN then
        pet:addMod(xi.mod.MPP, 15)
    elseif frame == xi.frames.STORMWAKER then
        pet:addMod(xi.mod.MPP, 13)
    end
end
...
attachment_object.onUnequip = function(pet)
    local frame = pet:getAutomatonFrame()
    if frame == xi.frames.HARLEQUIN then
        pet:delMod(xi.mod.MPP, 15)
    elseif frame == xi.frames.STORMWAKER then
        pet:delMod(xi.mod.MPP, 13)
    end
end
...
attachment_object.onUpdate = function(pet, maneuvers)
    ...
    xi.automaton.updateModPerformance(pet, xi.mod.REFRESH, 'mana_tank_iii_mod', power)
end
