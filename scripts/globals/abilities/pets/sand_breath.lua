...
ability_object.onUseAbility = function(pet, target, skill, action)
    ...
    local dmgmod = xi.mobskills.mobBreathMove(pet, target, 0.185, pet:getMainLvl() * 15, xi.magic.ele.EARTH) -- Works out to (hp/6) + 15, as desired
    dmgmod = (dmgmod * (1 + gear)) * deep
    pet:setTP(0)

    local dmg = AbilityFinalAdjustments(dmgmod, pet, skill, target, xi.attackType.BREATH, xi.damageType.EARTH, xi.mobskills.shadowBehavior.IGNORE_SHADOWS)
    target:takeDamage(dmg, pet, xi.attackType.BREATH, xi.damageType.EARTH)
    return dmg
end
