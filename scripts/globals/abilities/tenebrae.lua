...
ability_object.onUseAbility = function(player, target, ability)
    ...
    if target:getActiveRunes() > 0 and target:hasStatusEffect(xi.effect.TENEBRAE) then
        local effect = player:getStatusEffect(xi.effect.TENEBRAE)
    ...
    target:addStatusEffect(xi.effect.TENEBRAE,finalDmg,3,180)
    printf("final damage for en effect TENEBRAE = %s", finalDmg)

    return xi.effect.TENEBRAE
end
