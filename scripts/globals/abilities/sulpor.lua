...
ability_object.onUseAbility = function(player, target, ability)
    ...
    if target:getActiveRunes() > 0 and target:hasStatusEffect(xi.effect.SULPOR) then
        local effect = player:getStatusEffect(xi.effect.SULPOR)
    ...
    target:addStatusEffect(xi.effect.SULPOR,finalDmg,3,180)
    printf("final damage for en effect SULPOR = %s", finalDmg)

    return xi.effect.SULPOR
end
