...
ability_object.onUseAbility = function(player, target, ability)
    ...
    local burden = 15
    if (target:getStat(xi.mod.AGI) < target:getPet():getStat(xi.mod.AGI)) then
        burden = 20
    end

    local overload = target:addBurden(xi.magic.ele.WIND-1, burden)
    ...
    if (overload ~= 0) then
        ...
        target:addStatusEffect(xi.effect.WIND_MANEUVER, bonus, 0, utils.clamp(dur, 60, 300))

    end

    return xi.effect.WIND_MANEUVER
end
