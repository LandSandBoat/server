...
ability_object.onUseAbility = function(player, target, ability)
    ...
    local burden = 15
    if (target:getStat(xi.mod.STR) < target:getPet():getStat(xi.mod.STR)) then
        burden = 20
    end

    local overload = target:addBurden(xi.magic.ele.FIRE-1, burden)
    ...
    if (overload ~= 0) then
        ...
        target:addStatusEffect(xi.effect.FIRE_MANEUVER, bonus, 0, utils.clamp(dur, 60, 300))

    end

    return xi.effect.FIRE_MANEUVER
end
