...
ability_object.onUseAbility = function(player, target, ability)
    ...
    local burden = 15
    if (target:getStat(xi.mod.INT) < target:getPet():getStat(xi.mod.INT)) then
        burden = 20
    end

    local overload = target:addBurden(xi.magic.ele.ICE-1, burden)
    ...
    if (overload ~= 0) then
        ...
        target:addStatusEffect(xi.effect.ICE_MANEUVER, bonus, 0, utils.clamp(dur, 60, 300))

    end

    return xi.effect.ICE_MANEUVER
end
