...
xi.spells.damage.calculateBaseDamage = function(caster, target, spell, spellId, skillType, statDiff)
    ...
    if skillType == xi.skill.ELEMENTAL_MAGIC then
        if caster:isPC() then
            -- (M) In wiki.
            local spellMultiplier0   = pTable[spellId][M0]
            local spellMultiplier50  = pTable[spellId][M0 + 1]
            local spellMultiplier100 = pTable[spellId][M0 + 2]
            local spellMultiplier200 = pTable[spellId][M0 + 3]
            local spellMultiplier300 = pTable[spellId][M0 + 4]
            local spellMultiplier400 = pTable[spellId][M0 + 5]
            local spellMultiplier500 = pTable[spellId][M0 + 6]
    ...
    then
        ...
        local inflexionPoint  = pTable[spellId][I] -- I
        if statDiff <= 0 then
...
xi.spells.damage.calculateTMDA = function(caster, target, spell)
...
xi.spells.damage.useDamageSpell = function(caster, target, spell)
    ...
    local MTDR                 = xi.spells.damage.calculateMTDR(caster, target, spell)
    ...
    local TMDA                 = xi.spells.damage.calculateTMDA(caster, target, spell)
