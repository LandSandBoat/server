...
-- Structure:              [spellId] = { Tier, Main_Effect, Spell_Level, Base_Power, Base_Duration, Composure, Always_Overwrite, Tick_Seconds },
    ...
    [xi.magic.spell.FIRESTORM_II   ] = { 1, xi.effect.FIRESTORM_II,    99,    2,  180, true,  true,  0 },
    [xi.magic.spell.HAILSTORM_II   ] = { 1, xi.effect.HAILSTORM_II,    99,    2,  180, true,  true,  0 },
    [xi.magic.spell.WINDSTORM_II   ] = { 1, xi.effect.WINDSTORM_II,    99,    2,  180, true,  true,  0 },
    [xi.magic.spell.SANDSTORM_II   ] = { 1, xi.effect.SANDSTORM_II,    99,    2,  180, true,  true,  0 },
    [xi.magic.spell.THUNDERSTORM_II] = { 1, xi.effect.THUNDERSTORM_II, 99,    2,  180, true,  true,  0 },
    [xi.magic.spell.RAINSTORM_II   ] = { 1, xi.effect.RAINSTORM_II,    99,    2,  180, true,  true,  0 },
    [xi.magic.spell.AURORASTORM_II ] = { 1, xi.effect.AURORASTORM_II,  99,    2,  180, true,  true,  0 },
    [xi.magic.spell.VOIDSTORM_II   ] = { 1, xi.effect.VOIDSTORM_II,    99,    2,  180, true,  true,  0 },
...
xi.spells.enhancing.calculateEnhancingFinalPower = function(caster, target, spell, basePower, spellGroup, tier, spellEffect)
    ...
    -- -storm
    elseif
        (spellEffect >= xi.effect.FIRESTORM and spellEffect <= xi.effect.VOIDSTORM) or
        (spellEffect >= xi.effect.FIRESTORM_II and spellEffect <= xi.effect.VOIDSTORM_II)
    then
        -- 3 | 4 | 5 |  6 |  7 -- Values per merit without Loafers +2
        -- 4 | 6 | 8 | 10 | 12 -- Values per merit with Loafers +2

        -- 2 + | 1 | 2 | 3 |  4 |  5 -- Values per merit without Loafers +2 -> 2 + merit level
        -- 2 + | 2 | 4 | 6 |  8 | 10 -- Values per merit with Loafers +2    -> 2 + merit level * 2

        finalPower = finalPower + caster:getMerit(xi.merit.STORMSURGE) * (1 + caster:getMod(xi.mod.STORMSURGE_EFFECT))
    end
