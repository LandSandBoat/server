...
entity.onTrigger = function(player, npc)
    if (xi.settings.ENABLE_ABYSSEA == 1 and player:getMainLvl() >= 30) then
        ...
        if (HasStone >= 1 and player:getQuestStatus(xi.quest.log_id.ABYSSEA, xi.quest.id.abyssea.DAWN_OF_DEATH) == QUEST_ACCEPTED and
            player:getQuestStatus(xi.quest.log_id.ABYSSEA, xi.quest.id.abyssea.MEGADRILE_MENACE) == QUEST_AVAILABLE) then
