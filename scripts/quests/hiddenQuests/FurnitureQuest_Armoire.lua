    ...
    ['Moogle'] =
    {
        onTrigger = function(player, npc)
            if
                ...
                quest:getVar(player, 'RewardReadyTime') < GetSystemTime()
            then
    ...
    onEventFinish =
    {
        [30003] = function(player, csid, option, npc)
            if npcUtil.giveItem(player, quest.reward) then
                ...
                quest:setVar(player, 'RewardReadyTime', 0)
            end
        end,
    },
}
...
local armoirePlacedEvent =
{
    ['armoire'] =
    {
        onFurniturePlaced = function(player, item)
            quest:setVar(player, 'RewardReadyTime', GetSystemTime() + 60)
        end,
    },
}
