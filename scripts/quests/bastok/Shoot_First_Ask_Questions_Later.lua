            ...
            ['Cid'] =
            {
                onTrigger = function(player, npc)
                    if player:hasKeyItem(xi.keyItem.ANNALS_OF_TRUTH) then
                        ...
                        local hideReacquireMenuItem = (player:hasItem(xi.item.GUN_OF_TRIALS) or player:hasKeyItem(xi.keyItem.MAP_TO_THE_ANNALS_OF_TRUTH)) and 1 or 0
                        return quest:event(796, hideReacquireMenuItem) -- cont 1
                    end
                end,

                onTrade = function(player, npc, trade)
                    if npcUtil.tradeMatches(trade, { { xi.item.GUN_OF_TRIALS, 1 } }) then
