...
quests.flyers_for_regine.onTriggerAreaEnter = function(player, triggerArea)
    if player:getQuestStatus(xi.quest.log_id.SANDORIA, xi.quest.id.sandoria.FLYERS_FOR_REGINE) == xi.questStatus.QUEST_ACCEPTED then
...
quests.flyers_for_regine.onTrade = function(player, npc, trade, ffrId)
    if
        player:getQuestStatus(xi.quest.log_id.SANDORIA, xi.quest.id.sandoria.FLYERS_FOR_REGINE) == xi.questStatus.QUEST_ACCEPTED and
