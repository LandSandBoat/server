...
protected:
    ...
    void handle_error(std::error_code ec, std::shared_ptr<handler_session> self) override
    {
        if (self->sessionHash != "")
        {
            auto map = loginHelpers::authenticatedSessions_[self->ipAddress];
            ...
            if (it != map.end())
            {
                ...
                if (session.data_session == nullptr && session.view_session == nullptr)
                {
                    ...
                    if (loginHelpers::authenticatedSessions_[self->ipAddress].size() == 1)
                    {
                        loginHelpers::authenticatedSessions_.erase(loginHelpers::authenticatedSessions_.begin());
                    }
                }
            }
        }
