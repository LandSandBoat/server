...
: Application("connect", argc, argv)
{
    ...
    handler<auth_session> auth(*this, ioContext_, settings::get<uint32>("network.LOGIN_AUTH_PORT"));
    handler<view_session> view(*this, ioContext_, settings::get<uint32>("network.LOGIN_VIEW_PORT"));
    handler<data_session> data(*this, ioContext_, settings::get<uint32>("network.LOGIN_DATA_PORT"));

    // asio::steady_timer cleanup_callback(ioContext_, std::chrono::minutes(15));
