...
void TestRunner::overrideSettings() const
{
    const std::set<std::string> overrideCategories{ "logging", "main" };
    auto                        testSettings = lua["xi"]["settings"]["test"];

    for (const auto& category : overrideCategories)
    {
        if (testSettings[category].valid())
        {
            sol::table categorySettings = testSettings[category];

            // clang-format off
            categorySettings.for_each([&](const sol::object& key, const sol::object& value)
            {
                std::string keyStr = key.as<std::string>();
                const std::string settingPath = std::format("{}.{}", category, keyStr);

                // Publish to lua
                lua["xi"]["settings"][category][key] = value;

                // and update the C++ registry
                if (value.is<bool>())
                {
                    settings::set(settingPath, value.as<bool>());
                }
                else if (value.is<double>())
                {
                    settings::set(settingPath, value.as<double>());
                }
                else if (value.is<std::string>())
                {
                    settings::set(settingPath, value.as<std::string>());
                }
            });
            // clang-format on
        }
...
void TestRunner::captureLogger() const
{
    ...
    logging::SetPattern(settings::get<std::string>("logging.PATTERN"));
}
