    ...
    PEXCEPTION_POINTERS pExceptionInfo)
{
    ...
    if (m_hDumpFile)
    {
        ...
        if (pExceptionInfo->ExceptionRecord->ExceptionCode == EXCEPTION_ASSERTION_FAILURE && pExceptionInfo->ExceptionRecord->NumberParameters > 0)
        {
            ...
            additionalStream.BufferSize = (ULONG)strlen(reinterpret_cast<char const*>(pExceptionInfo->ExceptionRecord->ExceptionInformation[0])) + 1;
...
void WheatyExceptionReport::PrintSystemInfo()
{
    ...
    TCHAR sString[1024];
    if (_GetProcessorName(sString, (DWORD)std::size(sString)))
    {
    ...
    if (_GetWindowsVersion(sString, (DWORD)std::size(sString)))
        Log(_T("OS: %s"), sString);
    else
        Log(_T("OS: <unknown>"));
}
