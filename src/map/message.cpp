...
namespace message
{
                        ...
                        if (ret != SQL_ERROR && _sql->NumRows() == 2)
                        {
                            if (PInviter->PParty)
                            {
                                if (PInviter->PParty->m_PAlliance)
                                {
                                    ...
                                    {
                                        send(MSG_DIRECT, (uint8*)extra.data() + 6, sizeof(uint32), std::make_unique<CMessageStandardPacket>(PInviter, 0, 0, MsgStd::CannotBeProcessed));
                                    }
                                }
    ...
    void send(MSGSERVTYPE type, void* data, size_t datalen, std::unique_ptr<CBasicPacket>&& packet)
    {
        TracyZoneScoped;
    ...
    void send(uint32 playerId, std::unique_ptr<CBasicPacket>&& packet)
    {
        TracyZoneScoped;

        std::array<uint8, 4> packetData{};
        ref<uint32>(packetData.data(), 0) = playerId;
        message::send(MSG_DIRECT, packetData.data(), packetData.size(), std::move(packet));
    }

    void send(std::string const& playerName, std::unique_ptr<CBasicPacket>&& packet)
    {
        TracyZoneScoped;

        send(charutils::getCharIdFromName(playerName), std::move(packet));
    }
