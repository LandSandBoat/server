...
CInstance* CInstanceLoader::LoadInstance()
{
    ...
    if (!instance->Failed() && ret != SQL_ERROR /*&& sql->NumRows() != 0*/)
    {
        while (sql->NextRow() == SQL_SUCCESS)
        {
            ...
            PMob->setModifier(Mod::UDMGPHYS, (int16)sql->GetIntData(41));   // Modifier 387, base 10000 stored as signed integer. Positives signify less damage.
            PMob->setModifier(Mod::UDMGRANGE, (int16)sql->GetIntData(42));  // Modifier 390, base 10000 stored as signed integer. Positives signify less damage.
            PMob->setModifier(Mod::UDMGMAGIC, (int16)sql->GetIntData(43));  // Modifier 389, base 10000 stored as signed integer. Positives signify less damage.
            PMob->setModifier(Mod::UDMGBREATH, (int16)sql->GetIntData(44)); // Modifier 388, base 10000 stored as signed integer. Positives signify less damage.

            PMob->setModifier(Mod::FIRE_RES_RANK, (int8)(sql->GetIntData(45)));
            PMob->setModifier(Mod::ICE_RES_RANK, (int8)(sql->GetIntData(46)));
            PMob->setModifier(Mod::WIND_RES_RANK, (int8)(sql->GetIntData(47)));
            PMob->setModifier(Mod::EARTH_RES_RANK, (int8)(sql->GetIntData(48)));
            PMob->setModifier(Mod::THUNDER_RES_RANK, (int8)(sql->GetIntData(49)));
            PMob->setModifier(Mod::WATER_RES_RANK, (int8)(sql->GetIntData(50)));
            PMob->setModifier(Mod::LIGHT_RES_RANK, (int8)(sql->GetIntData(51)));
            PMob->setModifier(Mod::DARK_RES_RANK, (int8)(sql->GetIntData(52)));

            PMob->m_Element     = (uint8)sql->GetIntData(53);
            PMob->m_Family      = (uint16)sql->GetIntData(54);
            PMob->m_name_prefix = (uint8)sql->GetIntData(55);
            PMob->m_flags       = (uint32)sql->GetIntData(56);

            // Special sub animation for Mob (yovra, jailer of love, phuabo)
            // yovra 1: On top/in the sky, 2: , 3: On top/in the sky
            // phuabo 1: Underwater, 2: Out of the water, 3: Goes back underwater
            PMob->animationsub = (uint32)sql->GetIntData(57);

            // Setup HP / MP Stat Percentage Boost
            PMob->HPscale = sql->GetFloatData(58);
            PMob->MPscale = sql->GetFloatData(59);

            // TODO: Remove me
            // Check if we should be looking up scripts for this mob
            // PMob->m_HasSpellScript = (uint8)sql->GetIntData(60);

            PMob->m_SpellListContainer = mobSpellList::GetMobSpellList(sql->GetIntData(61));

            PMob->m_Pool = sql->GetUIntData(62);

            PMob->allegiance = static_cast<ALLEGIANCE_TYPE>(sql->GetUIntData(63));
            PMob->namevis    = sql->GetUIntData(64);

            uint32 aggro  = sql->GetUIntData(65);
            PMob->m_Aggro = aggro;
            ...
            PMob->m_MobSkillList  = sql->GetUIntData(66);
            PMob->m_TrueDetection = sql->GetUIntData(67);
            PMob->setMobMod(MOBMOD_DETECTION, sql->GetUIntData(68));

            PMob->setMobMod(MOBMOD_CHARMABLE, sql->GetUIntData(69));
