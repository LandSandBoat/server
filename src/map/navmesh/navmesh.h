...
// A single findPath() can exhaust the finite poly buffer before reaching a far destination, in
// which case Detour returns a best-effort path that stops short of `end`. isPartial flags this so
// callers (CPathFind) can chunk: walk the partial path, then re-request from the new position.
struct PathResult
{
