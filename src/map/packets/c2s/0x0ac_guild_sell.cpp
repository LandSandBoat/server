...
auto GP_CLI_COMMAND_GUILD_SELL::validate(Scheduler& scheduler, MapSession* PSession, const CCharEntity* PChar) const -> PacketValidationResult
{
...
void GP_CLI_COMMAND_GUILD_SELL::process(Scheduler& scheduler, MapSession* PSession, CCharEntity* PChar) const
{
    ...
    if (quantity != 0 && charItem->getQuantity() >= quantity)
    {
        ...
        {
            auditSale(scheduler, PChar, charItem->getID(), basePrice, quantity);
