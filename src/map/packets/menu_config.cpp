...
CMenuConfigPacket::CMenuConfigPacket(CCharEntity* PChar)
{
    ...
    ref<uint8>(0x04) = 0x18 | PChar->menuConfigFlags.byte1 | (PChar->nameflags.flags & (uint8)FLAG_INVITE ? (uint8)NFLAG_INVITE : 0);
    ref<uint8>(0x05) = PChar->menuConfigFlags.byte2 | (PChar->m_hasAutoTarget ? 0 : (uint8)NFLAG_AUTOTARGET >> 8);
