...
CSynthSuggestionListPacket::CSynthSuggestionListPacket(uint16 skillID, uint16 skillLevel, uint8 skillRank, uint16 resultOffset)
{
    ...
    const auto rset = db::preparedStmt(fmtQuery, craftName, craftName, minSkill, maxSkill, craftName, resultOffset);
    ...
    if (rset && rset->rowsCount() != 0)
    {
        ...
        while (rset->next())
        {
            ref<uint16>(itemIdOffset) = rset->get<uint32>(0);
...
CSynthSuggestionRecipePacket::CSynthSuggestionRecipePacket(uint16 skillID, uint16 skillLevel, uint8 skillRank, uint16 selectedRecipeOffset)
{
    ...
    const auto rset = db::preparedStmt(fmtQuery, craftName, craftName, minSkill, maxSkill, craftName, selectedRecipeOffset);
    ...
    if (rset && rset->rowsCount() != 0 && rset->next())
    {
        ...
        ref<uint16>(0x04) = rset->get<uint32>(10);
        ref<uint16>(0x06) = subcraftIDs[0];
        ref<uint16>(0x08) = subcraftIDs[1];
        ref<uint16>(0x0A) = subcraftIDs[2];
        ref<uint16>(0x0C) = rset->get<uint32>(9);
        ref<uint16>(0x0E) = rset->get<uint32>(0);
        ...
        for (auto i = 0; i < 8; ++i)
        {
            ...
            this_ingredient = rset->get<uint32>(11 + i);
