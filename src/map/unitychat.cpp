...
{
    sql->Async("UPDATE accounts_sessions SET unitychat = %u WHERE charid = %u", this->getLeader(), PChar->id);
...
bool CUnityChat::DelMember(CCharEntity* PChar)
{
    for (uint32 i = 0; i < members.size(); ++i)
    {
        ...
        {
            sql->Async("UPDATE accounts_sessions SET unitychat = 0 WHERE charid = %u", PChar->id);
