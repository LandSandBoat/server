...
void CCharRecastContainer::Add(RECASTTYPE type, uint16 id, uint32 duration, uint32 chargeTime, uint8 maxCharges)
{
    ...
    {
        sql->Async("REPLACE INTO char_recast VALUES (%u, %u, %u, %u);", m_PChar->id, recast->ID, static_cast<uint32>(recast->TimeStamp),
                  recast->RecastTime);
    }
}
...
void CCharRecastContainer::Del(RECASTTYPE type)
{
    ...
    if (type == RECAST_ABILITY)
    {
        sql->Async("DELETE FROM char_recast WHERE charid = %u;", m_PChar->id);
    }
}
...
void CCharRecastContainer::Del(RECASTTYPE type, uint16 id)
{
    ...
    sql->Async("DELETE FROM char_recast WHERE charid = %u AND id = %u;", m_PChar->id, id);
}
...
void CCharRecastContainer::DeleteByIndex(RECASTTYPE type, uint8 index)
{
    ...
    if (type == RECAST_ABILITY)
    {
        ...
        sql->Async("DELETE FROM char_recast WHERE charid = %u AND id = %u;", m_PChar->id, PRecastList->at(index).ID);
    }
    else
    {
...
void CCharRecastContainer::ResetAbilities()
{
    ...
    sql->Async("DELETE FROM char_recast WHERE charid = %u AND id != 0;", m_PChar->id);
}
