...
void CTransportHandler::InitializeTransport(IPP mapIPP)
{
    ...
    auto rset = db::preparedStmt(fmtQuery, mapIPP.getIP(), mapIPP.getIPString(), mapIPP.getPort());
    if (rset && rset->rowsCount())
    {
        while (rset->next())
        {
            ...
            zoneTown.ship.dock.zone = zoneutils::GetZone((rset->get<uint32>(1) >> 12) & 0x0FFF);

            zoneTown.ship.dock.p.x        = rset->get<float>(3);
            zoneTown.ship.dock.p.y        = rset->get<float>(4);
            zoneTown.ship.dock.p.z        = rset->get<float>(5);
            zoneTown.ship.dock.p.rotation = (uint8)rset->get<int32>(6);
            zoneTown.ship.dock.boundary   = (uint16)rset->get<int32>(7);
            zoneTown.ship.dock.prevzone   = (uint8)rset->get<int32>(8);

            zoneTown.npcDoor  = zoneutils::GetEntity(rset->get<uint32>(2), TYPE_NPC);
            zoneTown.ship.npc = zoneutils::GetEntity(rset->get<uint32>(1), TYPE_SHIP);
            if (!zoneTown.ship.npc)
            {
                ShowError("Transport <%u>: transport not found", (uint8)rset->get<int32>(0));
                continue;
            }

            zoneTown.ship.animationArrive = (uint8)rset->get<int32>(9);
            zoneTown.ship.animationDepart = (uint8)rset->get<int32>(10);

            zoneTown.ship.timeOffset      = (uint16)rset->get<int32>(11);
            zoneTown.ship.timeInterval    = (uint16)rset->get<int32>(12);
            zoneTown.ship.timeArriveDock  = (uint16)rset->get<int32>(14);
            zoneTown.ship.timeDepartDock  = zoneTown.ship.timeArriveDock + (uint16)rset->get<int32>(13);
            zoneTown.ship.timeVoyageStart = zoneTown.ship.timeDepartDock + (uint16)rset->get<int32>(15) - 1;
            ...
            {
                ShowError("Transport <%u>: door not found", (uint8)rset->get<int32>(0));
                continue;
            }
            if (zoneTown.ship.timeArriveDock < 10)
            {
                ShowError("Transport <%u>: time_anim_arrive must be > 10", (uint8)rset->get<int32>(0));
                continue;
            }
            if (zoneTown.ship.timeInterval < zoneTown.ship.timeVoyageStart)
            {
                ShowError("Transport <%u>: time_interval must be > time_anim_arrive + time_waiting + time_anim_depart", (uint8)rset->get<int32>(0));
                continue;
            }
    ...
    rset = db::preparedStmt(fmtQuery, mapIPP.getIP(), mapIPP.getIPString(), mapIPP.getPort());
    if (rset && rset->rowsCount())
    {
        while (rset->next())
        {
            ...
            voyageZone.voyageZone = zoneutils::GetZone((uint8)rset->get<uint32>(0));

            if (voyageZone.voyageZone != nullptr && voyageZone.voyageZone->GetID() > 0)
            {
                voyageZone.timeOffset   = (uint16)rset->get<int32>(1);
                voyageZone.timeInterval = (uint16)rset->get<int32>(2);

                voyageZone.timeArriveDock  = (uint16)rset->get<int32>(4);
                voyageZone.timeDepartDock  = voyageZone.timeArriveDock + (uint16)rset->get<int32>(3);
                voyageZone.timeVoyageStart = voyageZone.timeDepartDock + (uint16)rset->get<int32>(5);
            ...
            {
                ShowError("TransportZone <%u>: zone not found", (uint8)rset->get<int32>(0));
            }
        }
    }
}
