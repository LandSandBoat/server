...
// CPathFind orchestrates the pieces that own the work: a PathOwner (the moved entity, behind an
// interface), a Path (waypoints + cursor + geometry), a ChunkedPath (re-chunk controller), a
// NavPathBuilder (navmesh queries), and pathfind::stepTowards (kinematics). Holds the public request
// API, the per-tick FollowPath loop, and the FinishedPath continuation.
...
#include <map/entities/mobentity.h> // ROAMFLAG_WORM
...
auto CPathFind::PathToImpl(const position_t& point, uint8 pathFlags) -> bool
{
    ...
    // Single-destination PathTo (incl. scripted) always chunks: the navmesh poly buffer can't span a
    // far destination in one query, so without chunking a long path would walk only the first leg and
    // stop. (Multi-point authored routes go through AddPoints/PathThrough, not here.)
...
auto CPathFind::RoamAround(const position_t& point, float maxRadius, uint8 maxTurns, uint16 roamFlags) -> bool
{
    TracyZoneScoped;
...
auto CPathFind::PathTo(const position_t& point, uint8 pathFlags) -> bool
{
    ...
    // Don't overwrite an active scripted path with a non-scripted one - scripts must finish or be
...
auto CPathFind::FollowPath(timer::time_point tick) -> void
{
    ...
    // Paused (e.g. during a TP move / animation lock). Hold position, keep facing the battle target
    // if any, and bail until the pause elapses.
    ...
    // Sitting at a waypoint waiting out its `wait` duration. Once it elapses, advance to the next
    // waypoint (and complete the path if that was the last one).
    ...
    if (waitingAtPoint)
    {
        ...
        if (path_.consumed())
        {
            // FinishedPath fires onPathComplete, but only at the true end of the journey (not on a
            // re-chunk / roam turn / patrol loop).
            FinishedPath();
        }
        return;
    }
    ...
    // LimitDistance() caps total walked distance for this path; once exceeded, treat it as complete
    // in-place.
    ...
    // Walk forward through waypoints we've already arrived at, handling rotation snaps and `wait`
    // pauses. Stops at the first waypoint we still need to step toward.
    pathpoint_t targetPoint{};
    while (!path_.consumed())
    {
        targetPoint = path_.current();

        // Only the final waypoint honors the stop-short distance (distanceFromPoint_). Corner
        // waypoints must be reached precisely; otherwise we "arrive" up to distanceFromPoint_ early
        // and cut the corner, clipping the wall the navmesh corner was insetting us away from.
    ...
    // Stop short only for the last waypoint; round corners precisely (stopShort = 0) so we don't
    // cut across them.
    ...
    if (path_.consumed())
    {
        // FinishedPath fires onPathComplete, only at the true end (not on a chunk/turn/patrol
        // continuation).
        FinishedPath();
        onPoint_ = true;
    }
...
{
    // External callers (e.g. trusts stepping straight at a target) honor the configured stop-short
    // distance, which is 0 unless a range was set.
...
auto CPathFind::StepToInternal(const position_t& pos, bool run, float stopShort) -> void
{
    ...
    const float speed = [&]() -> float
    {
        ...
        if (owner_->isMobEntity() && baseSpeed == 0 && (roamFlags_ & ROAMFLAG_WORM))
        {
            return 20.0f;
        }
    ...
    // Free-step toward `pos`. Kinematics live in pathfind_step so tests can exercise the exact math;
    // stepTowards() also faces the owner toward `pos`.
    position_t& ownerPos = owner_->position();
...
auto CPathFind::BuildDirectPath(const position_t& end) -> bool
{
    ...
    // Single-waypoint "path" straight to the destination, ignoring the navmesh (may clip through
    // geometry). Opt-in wallhacking fallback; see the commented call in FindPathInternal.
    path_.assign({ pathpoint_t{ end, 0s, false } }, false);
    return true;
}

auto CPathFind::FindRandomPath(const position_t& start, float maxRadius, uint8 maxTurns, uint16 roamFlags) -> bool
{
    TracyZoneScoped;
    TracyZoneString(owner_->name());
    ...
    // Path to the first turn; later turns chain in FinishedPath(). Random roams don't use chunked
    // pathing, and partial-path detection is irrelevant since there's no specific destination.
...
{
    // Stop-short applies only to the final waypoint. Corner waypoints are reached precisely (0.1y)
    // so we don't arrive early and cut the corner. Final point: 0.1y for an exact target, else the
    // requested stop-short plus a small slack to avoid oscillating at the boundary.
...
{
    // For chunked paths the Path's last point is only the current chunk's end, not the real
    // destination. Return the chunked target so callers comparing GetDestination() against the
    // eventual goal (e.g. mob_controller's needNewPath drift check) are correct for both chunked
    // and single-shot paths.
    if (chunked_.active())
    {
...
auto CPathFind::ChunkCount() const -> int
{
    ...
    // Diagnostic - handy in-game and used by the pathfind tests.
    return chunkCount_;
}
...
{
    // Patrol paths may legitimately exceed kMaxPathPoints (long scripted routes); cap any other
    // caller so a runaway path doesn't pin the AI on one mob for minutes.
...
{
    // The chunk just consumed was partial - silently request the next chunk toward the eventual
    // destination. NOT a completion, so it must NOT fire onPathComplete; checked first, before the
    // hook below, so chunk boundaries stay invisible to scripts. turnPoints_ is empty for chunked
    // PathTo, so this can't race the roam-turn logic.
    if (chunked_.active() && path_.partial() && TryRequestNextChunk())
    {
        return;
    }

    // A genuine completion: single-shot arrival, roam turn, or patrol lap. Fire the script hook
    // FIRST - its handler may set up the next path (e.g. a patrol NPC re-pathing on completion),
    // and the continuation logic below must see that updated state.
