...
auto ChunkedPath::evaluate(float distanceToTarget) -> Step
{
    ...
    // Effectively reached the destination. PathInRange uses range as the stop-short distance; plain
    // PathTo has none, so a small slack covers the navmesh dropping us within a yalm of the end.
    ...
    // Stall detection: if the previous chunk didn't move us closer, the navmesh is circling an
    // obstacle we can't clear. Bail after a few consecutive non-progressing chunks.
