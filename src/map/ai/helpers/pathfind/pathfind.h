...
#include <map/ai/helpers/pathfind/pathfind_constants.h>
...
public:
    ...
    // General/tests: take ownership of an injected PathOwner (e.g. TestPathOwner).
    explicit CPathFind(std::unique_ptr<pathfind::PathOwner> owner);

    ~CPathFind();

    // Walk to a random point around the given point.
    auto RoamAround(const position_t& point, float maxRadius, uint8 maxTurns, uint16 roamFlags = 0) -> bool;
    ...
    // Instantly move the entity to `point`, offset by `maxDistance` (default 2y). Skips the navmesh -
    // caller must ensure the destination is valid.
    ...
    // Pause pathing for the given duration; zero pauses indefinitely (until Unpause()).
    // While paused the entity holds position but keeps facing its battle target, if any.
    ...
    // True when the path length is within `ratio` x straight-line distance to the destination
    // (not an unreasonable detour). False if no path is active, the path is partial (chunked),
    // or the route exceeds `ratio` x direct distance.
    auto IsPathDirect(float ratio = 2.0f) const -> bool;

    // Navmesh legs taken by the current PathTo journey (1 = single query, >1 = chunked).
    // Diagnostic - useful in-game and in the pathfind tests.
...
private:
    ...
    // Core of PathTo: activates chunked pathing, calls FindPathInternal. Does NOT call Clear()
    // (all public entry points clear first) and does NOT guard against scripted paths.
    ...
    auto FindRandomPath(const position_t& start, float maxRadius, uint8 maxTurns, uint16 roamFlags) -> bool;

    // Core of StepTo: step toward `pos`, settling `stopShort` yalms short of it. StepTo forwards
    // distanceFromPoint_; FollowPath passes stop-short only for the final waypoint, 0 for corners.
    ...
    // Re-request the next chunk after a partial path. True if a new chunk was set up, false if
    // we've arrived (or pathing failed).
    auto TryRequestNextChunk() -> bool;

    // True when the owner is within the arrival threshold of `pos`. isFinalPoint selects the
    // stop-short distance (final) vs a tight 0.1y threshold (intermediate corner).
    auto AtPoint(const position_t& pos, bool isFinalPoint = true) const -> bool;

    // The moved entity (production: an EntityPathOwner). Owned here so the abstraction stays inside
    // the pathfind module - see path_owner.h.
    std::unique_ptr<pathfind::PathOwner> owner_;

    // The active path being walked (waypoints + cursor + geometry).
    pathfind::Path path_;

    // Re-requests the next leg of a partial path toward the eventual destination (set by
    // PathTo / PathInRange). See chunked_path.h.
    ...
    // Path is held while tick < unpauseTime_ (see Pause/Unpause). time_point::min() = not paused.
    ...
    // Legs taken by the current PathTo journey (1 = single query, >1 = chunked). See ChunkCount().
    int chunkCount_{ 0 };
};
