...
int32 CalculateEnspellDamage(CBattleEntity* PAttacker, CBattleEntity* PDefender, uint8 Tier, uint8 element, CItemWeapon* pWeaponHit)
{
    ...
    // All non-weapon enspell dmg % (armor/etc)
    mult += (float)nonWeaponPct / 100.0f;

    // Composure bonus: only RDM main, only Tier I/II elemental (Fire..Water)
    if (PChar &&
    ...
    // This hand's weapon-only enspell dmg % (Crocea Mors Path C etc)
