    ...
    void BuildTrust(uint32 TrustID)
    {
        ...
        if (ret != SQL_ERROR && _sql->NumRows() != 0)
        {
            while (_sql->NextRow() == SQL_SUCCESS)
            {
                ...
                // resistances
                trust->slash_sdt  = (uint16)(_sql->GetIntData(31));
                trust->pierce_sdt = (uint16)(_sql->GetIntData(32));
                trust->hth_sdt    = (uint16)(_sql->GetIntData(33));
                trust->impact_sdt = (uint16)(_sql->GetIntData(34));
